---
title: ScopeQL query syntax
---

ScopeQL queries are written in pipeline order: start with data, then filter, transform, aggregate, order, and limit the result.

ScopeQL uses relational query concepts, but it is not a PostgreSQL or MySQL dialect. Use this page to check ScopeDB's query clause order and supported clause shapes.
...
VALUES
    ('checkout', 42.50),
    ('signup', NULL)
SELECT $0 AS event_type, $1 AS amount;
```
